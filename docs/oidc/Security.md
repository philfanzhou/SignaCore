...
| Oversized, non-form, compressed, or ambiguously encoded request body on the standard endpoints | One bounded single read of at most 16 KiB counted on the bytes actually read (never on `Content-Length`), strict UTF-8, single percent-decode, fixed 400/503 after budget admission, and no credential lookup or side effect for a rejected body | [outer bounded form read](./CanonicalSemanticModel.md#outer-bounded-form-read-of-post-oauth2token-and-post-oauth2revoke) | [#336](https://github.com/philfanzhou/SignaCore/issues/336) |
