...
| Oversized, non-form, compressed, or ambiguously encoded request body on Token, Revoke, or Logout preparation | One bounded read accepting at most 16 KiB, with a one-byte oversize probe, counted on the bytes actually read (never on `Content-Length`), strict UTF-8, single percent-decode, fixed 400/503 after budget admission, and no credential lookup or side effect for a rejected body | [outer bounded form read](./CanonicalSemanticModel.md#outer-bounded-form-read-of-token-revoke-and-logout-preparation) | [#336](https://github.com/philfanzhou/SignaCore/issues/336), [#337](https://github.com/philfanzhou/SignaCore/issues/337) |
