...
slashes cannot reach a routable shape around the gate. A non-form media type never selects the
form-consuming action at all and keeps the action-selection 415 unchanged — its body is never read.
...
| Non-UTF-8 charset, an extra content-type parameter, `Content-Encoding`, or malformed percent/UTF-8 | Same fixed marker | Same fixed `400`; no `Location`, no input echo |
...
Sensitive values exist only in the raw request buffer and the current request's form; the buffer is
zeroed after parsing, and no password, code, verifier, token, secret, authorization header, or
unknown value reaches the logs, metrics, or either fixed error body. Managed strings cannot be
cleared on the spot.
