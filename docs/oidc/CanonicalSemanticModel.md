...
| `PS-05` | Authorization code | Row contains code digest, client, account, session FK, exact redirect URI, canonical scope, nonce, S256 challenge, auth time, created/expiry/consumed times, nullable `refresh_family_id` | Unique digest. Family link points to the family root token id and is set only with root creation in `EV-21`; no-refresh success commits null |
...
cascade that erases evidence or silently changes a child artifact's state.
...
| `AC-03` | #50 code persistence | Codes can be stored and atomically consumed by internal tests | None |
| `AC-04` | #93 authorization validation | Requests and safe errors can be validated | None |
| `AC-05` | #94 authorization orchestration | `/oauth2/authorize` can issue a bound code | None; a code alone is not a usable advertised flow |
...
| `AC-09` | #67 identity-session persistence/lifecycle and #69 state propagation | Cross-instance session authority and disablement behavior are enforceable | No new metadata; prerequisites must be complete before dependent capability is considered production-ready |
