...
1. **Bootstrap Apps 文件预置**：首次部署时由 `DatabaseInitializer` 读取 `data/bootstrap-apps.json` 一次性预置。适用于基础应用（如 Teacher Portal）的初始凭据部署。AppId 为人工指定的固定值，AppSecret 由部署脚本生成或测试凭据。
