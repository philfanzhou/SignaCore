...
An authenticated application registers an allow-listed callback URL used to enrich token claims.
...
Only authenticated applications may register callbacks. URLs must use an allowed scheme and host and must pass the configured domain policy.
...
The feature owns or reads app_registrations. Database access remains behind repository interfaces and the unit-of-work/IdentityDbContext boundaries.
