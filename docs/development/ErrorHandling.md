...
| 字段类型 | 脱敏规则 | 示例 |
|----------|---------|------|
| 手机号 | 保留前 3 位 + 后 4 位，中间用 `****` 替换 | `138****1234` |
| 微信 OpenId | 保留前 4 + 后 4 位，中间用 `****` 替换 | `o1Qx****wxyz` |
...
所有限流拦截器（gRPC `RateLimitingInterceptor`、HTTP JWKS 端点限流器）触发拒绝时，必须输出 Warning 级别日志，包含客户端 IP 与命中限流策略，便于在 Loki 上检索攻击或异常调用模式。
