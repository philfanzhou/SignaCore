...
public class WechatValidator : IIdentityValidator
{
    ...
    public async Task<ValidationResult> ValidateAsync(ValidationRequest request)
    {
        ...
        if (account == null)
        {
            _logger.LogWarning("WeChat validation failed: WeChat not bound to any account, OpenId={OpenId}", openId);
            return ValidationResult.Failure("WeChat is not bound to any account");
        }

        if (!account.IsActive)
        {
            _logger.LogWarning("WeChat validation failed: account not found or disabled, OpenId={OpenId}", openId);
            return ValidationResult.Failure("Account is disabled");
        }

        _logger.LogInformation("WeChat validated successfully: OpenId={OpenId}, AccountId={AccountId}", openId, account.Id);
