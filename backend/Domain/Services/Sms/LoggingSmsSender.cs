...
public class LoggingSmsSender : ISmsSender
{
    ...
    public Task SendAsync(string phone, string code)
    {
        ...
        _logger.LogInformation("[SMS-DEV] Phone={Phone}, Code={MaskedCode} — SMS sent (logging only in dev mode)", phone, maskedCode);
...
public class ThrowingSmsSender : ISmsSender
{
    ...
    public Task SendAsync(string phone, string code)
    {
        _logger.LogError("SMS sending requested in production but no real SMS provider is configured. Phone={Phone}", phone);
