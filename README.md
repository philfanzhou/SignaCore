...
The API listens on the URLs in `src/SignaCore.Host/Properties/launchSettings.json`. Useful endpoints include `/health/live`, `/health/ready`, `/.well-known/openid-configuration`, `/.well-known/jwks` (also reachable as `/.well-known/jwks.json`), `/metrics` (requires registered application credentials), `/oauth2/token`, `/api/auth/token`, and `/admin`.
