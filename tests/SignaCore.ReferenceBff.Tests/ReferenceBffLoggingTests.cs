...
public sealed class ReferenceBffLoggingTests
{
    ...
    [InlineData(false)]
    [InlineData(true)]
    public async Task BrowserFlow_AllConsoleEntriesExcludeCanariesAndExceptions(bool failAudit)
    {
        await using var database = await LogDatabase.CreateAsync();
        ...
        await using var host = BffTestServer.Create(SignaCoreHostFixture.Authority, SignaCoreHostFixture.ClientId,
            ...
            identity.Host.Server.CreateHandler(), databaseProvider: "SQLite", databaseConnectionString: database.ConnectionString,
        ...
        var canaries = new[] { SignaCoreHostFixture.Password, SignaCoreHostFixture.ClientSecret, terminal.Code!.Reveal(), cookie,
            access, ticket.Properties.GetTokenValue("id_token")!, csrf, "Bearer " + access, database.ConnectionString };
        ...
        Assert.Equal(failAudit ? HttpStatusCode.ServiceUnavailable : HttpStatusCode.NoContent, setup.StatusCode);
        using var admin = await browser.Bff.GetAsync("/bff/admin", TestContext.Current.CancellationToken);
        Assert.Equal(failAudit ? HttpStatusCode.Forbidden : HttpStatusCode.OK, admin.StatusCode);
        ...
        var output = capture.Text;
        Assert.All(canaries.Append(logoutCsrf), canary => Assert.True(canary.Length > 0 && !output.Contains(canary, StringComparison.Ordinal)));
        var events = output.Split('\n').Where(line => line.Contains("Reference BFF operation completed.", StringComparison.Ordinal)).ToArray();
        Assert.All(events, AssertIdentity);
        foreach (var operation in new[] { "Login", "UserInfo", "Authorization", "Setup", "Logout" })
    ...
    public async Task ProgramCommandBranch_RejectsRedirectedTerminalBeforeBuildingHost(bool configured, string operation)
    {
        ...
        if (configured)
        {
            ...
            start.Environment["ReferenceBffDatabase__ConnectionString"] = "Data Source=:memory:";
        }
        ...
        Assert.Equal("Setup code usage or terminal rejected.", (await stderr).Trim());
    }
